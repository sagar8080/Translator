...
im = Image.open('uiet.jpg')
tkimage = ImageTk.PhotoImage(im)
myvar=Tkinter.Label(root,image = tkimage)
myvar.place(relx=0.5, rely=0.65, anchor=CENTER)

T = Text(root, height=2, width=45)
T.place(relx=0.5, rely=0.95, anchor=CENTER)
T.insert(END, "SAGAR DAS UE158089 // ENTER REMARKS")
...
def main():
	...
	#match letter I = cv2.imread('I.jpg',0)
	...
	while(cap.isOpened()):
	    ...
	    if k == ord('s'):
			...
			break
x0 = Label(root, text=' CLICK HERE FOR INSTRUCTIONS ')
x0.config(width =75)
x0.config(font=("Helvetica" , 20))
x0.place(relx=0.5, rely=0.05,anchor=CENTER)

b1 = Button(root, text="Instructions",command=call_back,bg="lightgreen")
b1.config(font=("Helvetica", 15,"bold"))
b1.place(relx=0.5, rely=0.10,anchor=CENTER)

x = Label(root,text="RUN THE APP")
x.config(width=75)
x.config(font=("Helvetica", 20))
x.place(relx=0.5, rely=0.15,anchor=CENTER)

b2 = Button(root, text="Run",command=main,bg="gold1")
b2.config(font=("Helvetica", 15,"bold"))
b2.place(relx=0.5, rely= 0.20,anchor=CENTER)

z = Label(root,text="EXIT")
z.config(width=75)
z.config(font=("Helvetica", 20))
z.place(relx=0.5, rely=0.25,anchor=CENTER)


b3 = Button(root, text="Exit",command=exit,bg="salmon")
b3.config(font=("helvetica", 15,"bold"))
b3.place(relx=0.5, rely=0.30,anchor=CENTER)
